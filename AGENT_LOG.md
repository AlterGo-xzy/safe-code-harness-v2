...
- 人工干预与教训：协调会话未编辑功能源码，只调度审查、运行独立验证并回填证据。接口“不回显”不足以保证安全；异常 cause、traceback 和错误日志也必须被视为凭据数据流并有失败回归。任务完成后仍须按 `finishing-a-development-branch` 创建单独 draft PR 并在此日志补真实链接。
