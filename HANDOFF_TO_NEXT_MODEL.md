...
GitHub PR #1–#17 已全部普通 merge；最后一次完整运行时验证的 GitHub/NJU `main` 基线为 `a205a231cf30e09bc3c86c55a394572a5f309029`。该提交的 [GitHub CI 31390746883](https://github.com/AlterGo-xzy/safe-code-harness-v2/actions/runs/31390746883) 成功执行 backend、三份机制 demo、frontend unit/build、Chromium E2E 和 Docker build；成功 CI 触发的 [GHCR publish 31390905424](https://github.com/AlterGo-xzy/safe-code-harness-v2/actions/runs/31390905424) 也成功。使用空 Docker 配置匿名拉取 `ghcr.io/altergo-xzy/safe-code-harness-v2:sha-c633003a06ad8073852f9125e3e195635f159bbb`（digest `sha256:efebd5cc0277b73ddbfbecf00ad843af1c127b3ba31e0395f3de6b46825694d2`），临时 loopback 容器的 `/` 与 `/api/runs` 皆 HTTP 200。若本交接文件随后只增加文档补正，接手者必须用 `git rev-parse origin/main` 取得实际最新 SHA，不能把文档提交当作重新跑过上述运行时验证。
