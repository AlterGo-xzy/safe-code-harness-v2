...
13. 任务 13 Task 1 已在 `codex/t13-demos-e2e` 完成真实 API 集成：新测试先 RED（Task 8-only `create_app` 不接受 `secret_store`），再以 `ec613df`、`1664aa2` 顺序合并 Task 9/10 reviewed branches；集成测试与过程记录提交为 `fd38e6a`。app factory 统一拥有 `run_service`、`planner_configuration`、`workspace_registry` 并注册 runs/config/workspaces routers；reviewed 安全源文件与测试相对来源 branch 无 diff。focused 为 `1 passed`，完整 backend 为 `146 passed`，只有既有 TestClient warning。Task 2 随后由 fresh implementer `/root/t13_task2_implementer` 严格先新增 `test_demos.py`，RED 为缺少 `scripts.run_approval_demo` 的 `ModuleNotFoundError`；最小实现只使用当前仓库代码，未读取或迁入旧项目，源代码与首次证据提交为 `26f9855`。首次审查发现审批 transcript 仍为固定字面量、PowerShell 不传播子进程非零退出、临时目录清理吞掉失败；三项均先新增 RED 回归后以 `7243dfc` 修复：transcript 从真实 pending/completed snapshots 投影并拒绝等待时执行或批准晚于执行，PowerShell 在每个 child 后检查 `$LASTEXITCODE`，清理失败抛固定异常。修复 focused 为 `10 passed in 0.47s` 且正常 `.\scripts\run_demos.ps1` 仍输出三份稳定 JSON；完整 backend 为 `156 passed, 1 warning`（既有 TestClient 弃用），diff check 无输出、凭据候选计数为 0。复审仍待执行。新增的 root `README.md` 仅记录离线 demo 命令和边界；Makefile 的 Unix-like `demos` target 未在本 Windows 主机假称已运行。Task 3 的真实浏览器/320px E2E 仍未执行；策略扩展继续延后。
