...
- 当前源码范围：任务 1-8 的 Harness 核心与受治理运行/审批 API、任务 9 的离线安全 Planner 配置、任务 10 的安全 ZIP 上传和隔离工作区均已完成并经审查；任务 13 Task 1 已在本分支把三组真实 API 合并到同一 app factory，focused `1 passed`、完整 backend `146 passed`。任务 13 Task 2 已以先失败后最小实现完成三份离线、稳定 JSON 机制演示；此前 focused `10 passed`、完整 backend `156 passed, 1 warning` 是历史验证证据。第二次独立复审为 Critical 0、Important 1、Minor 2；其三个发现已用 RED→最小修复关闭，并实际得到 demo `12 passed`、正常 Windows 入口三份安全 JSON、完整 backend `158 passed, 1 warning`、diff check 无输出、凭据候选计数 0。第三次 scoped re-review 确认交接/过程文档修正后为 PASS（Critical/Important/Minor 均为 0）。**Task 13 Task 2 已完成；下一步是 Task 3 的真实浏览器 E2E 与 320px 验证。**任务 11 已完成只读中文运行工作台，任务 12 已在其前端基线完成审批决定、Planner 配置和 ZIP 上传的安全 UI。Task 9 的 Windows Credential Manager-only 存储不配置真实 API key；Task 12 最终审查的 6 个 Minor 均已关闭，完整前端为 10 files/48 tests。用户批准将策略扩展延后：任务 12 没有策略 API/UI、假保存或 workspace 切换。任务 12 已创建目标为任务 11 的 [stacked draft PR #12](https://github.com/AlterGo-xzy/safe-code-harness-v2/pull/12)，保留分支/worktree 等待审查；任务 13 的真实浏览器 E2E 与 320px 验证仍未执行。详细续开发指令见 `HANDOFF_TO_NEXT_MODEL.md`。
...
13. 任务 13 Task 1 已在 `codex/t13-demos-e2e` 完成真实 API 集成：新测试先 RED（Task 8-only `create_app` 不接受 `secret_store`），再以 `ec613df`、`1664aa2` 顺序合并 Task 9/10 reviewed branches；集成测试与过程记录提交为 `fd38e6a`。app factory 统一拥有 `run_service`、`planner_configuration`、`workspace_registry` 并注册 runs/config/workspaces routers；reviewed 安全源文件与测试相对来源 branch 无 diff。focused 为 `1 passed`，完整 backend 为 `146 passed`，只有既有 TestClient warning，独立审查 PASS。Task 2 随后由 fresh implementer `/root/t13_task2_implementer` 严格先新增 `test_demos.py`，RED 为缺少 `scripts.run_approval_demo` 的 `ModuleNotFoundError`；最小实现只使用当前仓库代码，未读取或迁入旧项目，源代码与首次证据提交为 `26f9855`。首次审查发现审批 transcript 仍为固定字面量、PowerShell 不传播子进程非零退出、临时目录清理吞掉失败；三项均先新增 RED 回归后以 `7243dfc` 修复：transcript 从真实 pending/completed snapshots 投影并拒绝等待时执行或批准晚于执行，PowerShell 在每个 child 后检查 `$LASTEXITCODE`，清理失败抛固定异常。此前 focused `10 passed in 0.47s`、Windows JSON 入口和完整 backend `156 passed, 1 warning`（既有 TestClient 弃用）均为历史证据。**第二次独立复审为 Critical 0、Important 1、Minor 2；本轮 `7bdd85d` 已以 RED→最小修复拒绝等待/批准前的 `tool_failed` 等工具结果，并让 PowerShell 回归证明失败 child 后的 later child 不运行。实际验证为 demo `12 passed in 0.46s`、正常 Windows 入口、完整 backend `158 passed, 1 warning`、diff check 无输出、凭据候选计数 0。第三次独立审查仅指出旧交接/过程文档未同步上述事实；`5bf57a3` 文档修正的 scoped re-review 已 PASS（Critical/Important/Minor 均为 0），故 Task 2 已完成，可开始 Task 3。**新增的 root `README.md` 仅记录离线 demo 命令和边界；Makefile 的 Unix-like `demos` target 未在本 Windows 主机假称已运行。Task 3 的真实浏览器/320px E2E 仍未执行；策略扩展继续延后。完整交接见 `HANDOFF_TO_NEXT_MODEL.md`。

## 2026-08-10 最新 Task 13 状态（优先于上方任务 13 过程叙述中的暂态文字）

Task 13 Task 2 的第三次 scoped re-review 已 PASS（Critical/Important/Minor 均为 0），因此 Task 2 已完成；其当前有效验证是 demo `12 passed`、正常 Windows JSON 入口、完整 backend `158 passed, 1 warning`、diff check 无输出、凭据候选计数 0。下一步为 Task 3 的真实浏览器 E2E 与 320px 验证；任务 14、15 和用户延后的策略扩展仍未开始。
