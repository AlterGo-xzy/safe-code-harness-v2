...
- [x] **步骤 5：最终两阶段审查、分支收尾与提交**：本地两阶段审查、GitHub main CI、GHCR publish/匿名 pull-run、GitLab 最后绿和 Railway Mock URL 均已有真实证据；最终证据 PR #16 已 merge `d56516f`，并将 `origin/main` 快进推送至 NJU Git `main`。认证/TLS 的真实 key 生产部署由用户明确延后，作为扩展而非本项目必做交付。
