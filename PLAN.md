...
- 旧项目：Task 1 没有读取或复制旧代码，Task 2 没有读取或复制旧源码。旧 `D:\2026_summer_project\frontend\src\components\ConfigPanel.tsx`、`WorkspaceUploadPanel.tsx` 和 `backend\src\safe_code_harness\api\routes_config.py` 仅在设计文档中登记为未来参考；未迁入策略、localStorage、路径展示或自定义 workspace 事件。
