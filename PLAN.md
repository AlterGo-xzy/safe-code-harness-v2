...
**Task 2 演示检查点（第三次审查文档修正 scoped re-review 待进行，禁止进入 Task 3）：** fresh implementer `/root/t13_task2_implementer` 先创建 `backend/tests/integration/test_demos.py`；初始 RED 命令 `.\.venv\Scripts\python.exe -m pytest backend\tests\integration\test_demos.py --basetemp .pytest-tmp\task13-demos-red -q` 因三份脚本缺失而以 `ModuleNotFoundError: scripts.run_approval_demo` 失败。最小实现只复用本仓库的 `CommandGuard`/`RuntimePolicy`、`AgentLoop`/`MockLLM`/反馈/记忆和 `RunService`/`ApprovalStore`，反馈脚本在临时目录中运行并在 `finally` 清理；输出只投影稳定结构，不含命令、临时路径或敏感内容。初始 GREEN 为 `6 passed in 0.23s`，`.\scripts\run_demos.ps1` 按固定顺序输出三份 JSON；CLI 测试解析 JSON 并拒绝 `C:`、`D:`、`secret`。首次审查发现 approval demo 实际返回固定三行而没有从 event sequence 派生、PowerShell child 失败仍 exit 0、`shutil.rmtree(ignore_errors=True)` 会静默掩盖清理失败。三条补充 RED 依次为缺少 snapshot 投影 helper、PowerShell failing child 返回 0、缺少 cleanup helper；并额外记录现有 completed snapshot 会有两个 `approval_approved` 投影，故以执行前最后一个批准事件验证顺序。最小修复后 approval subset `4 passed`、PowerShell subset `1 passed`、cleanup subset `1 passed`、完整 demo suite `10 passed in 0.47s`，正常 Windows 入口成功。完整 backend 为 `156 passed, 1 warning`（既有 TestClient 弃用），`git diff --check` 无输出、凭据候选计数为 0；修复提交 `7243dfc fix: harden deterministic demo evidence`。新增 `Makefile` 的 Unix-like `demos` target 和最小 README 离线说明；Windows 无 GNU make，故未假称运行 `make demos`。未读、复制或咨询旧项目代码。**第二次独立复审为 Critical 0、Important 1、Minor 2，三项已闭环：`7bdd85d` 以两项真实 RED 拒绝 waiting/批准前的 `tool_failed`，并让 PowerShell 回归证明 failing first child 后的写 marker later child 不运行；focused 子集 `4 passed`、完整 demo `12 passed in 0.46s`、正常 Windows 入口、完整 backend `158 passed, 1 warning`、diff check 无输出且凭据候选计数为 0。第三次独立审查仅发现交接/过程文档未同步这些事实；本轮文档修正 scoped re-review 待进行，故此修改不得写成 Task 2 完成；Playwright 或 320px 验证未开始。**
