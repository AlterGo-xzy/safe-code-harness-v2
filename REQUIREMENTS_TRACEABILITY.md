...
| A-6-2 | 三项机制演示：危险阻断、失败反馈改变动作、主贡献行为 | 部分完成（任务 13 Task 2，修复待复审） | `26f9855` 的三份 `scripts/run_*_demo.py` 及 Windows 入口已产生稳定 JSON：危险命令被阻止、失败反馈先于修复动作、approval transcript 从真实 snapshots 派生并证明等待时未执行且批准在执行前；PowerShell 与 cleanup failure 不再静默继续。复审仍是完成门槛。 |
